...
describe("exporters", () => {
  ...
  describe("renderToImage", () => {
    ...
    it("respects abort signal before sensitive check", async () => {
      ...
      const el = document.createElement("div");
      await expect(
        renderToImage(el, makeOptions({ signal: ctrl.signal })),
      ).rejects.toThrowError(/Export aborted/);
    });
    ...
    it("does not check abort after toBlob resolves", async () => {
      ...
      await expect(
        renderToImage(
          el,
          makeOptions({ allowSensitiveData: true, signal: ctrl.signal }),
        ),
      ).resolves.toBeInstanceOf(HTMLCanvasElement);
  ...
  describe("createCanvasExporter", () => {
    ...
    it("passes format-specific mime type and quality to canvas toBlob", async () => {
      ...
      const jpegExporter = createCanvasExporter(
        "jpeg",
        "image/jpeg",
        "jpg",
        0.92,
      );
      const webpExporter = createCanvasExporter(
        "webp",
        "image/webp",
        "webp",
        0.9,
      ...
      await jpegExporter.export(
        el,
        makeOptions({ format: "jpeg", allowSensitiveData: true }),
      );
      await webpExporter.export(
        el,
        makeOptions({ format: "webp", allowSensitiveData: true }),
      );

      expect(toBlobSpy).toHaveBeenNthCalledWith(
        1,
        expect.any(Function),
        "image/jpeg",
        0.92,
    ...
    it("uses default filename with timestamp when none provided", async () => {
      ...
      const result = await exporter.export(
        el,
        makeOptions({ allowSensitiveData: true }),
      );
